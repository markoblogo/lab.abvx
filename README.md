...
- [SET](https://lab.abvx.xyz/tools/set/) for one CI entrypoint that keeps repo AI surfaces up to date and exports proposal-lifecycle, review-hint, optional shadow-first governance, loop-readiness, bounded orchestration, loop-hardening, and design-taste-review contracts for downstream runners.
...
- [ABVX Agent Skills](https://lab.abvx.xyz/tools/abvx-agent-skills/) — Portable, validation-gated skills for coding, research, frontend, audits, debugging, token economy, repo onboarding, evidence-backed product context, bounded Planner/Reviewer/Executor orchestration, opt-in project hook guardrails, bounded L1/L2 loop run contracts, proposal-first growth loops, local friction ledgers, reversible tasks, context and tool-contract review, bounded design-taste routing, delivery gates, handoffs, browser verification, and standalone HTML artifacts, installable from PyPI as `abvx-agent-skills`.
